...
function updateMetaJsonFiles() {
  console.log('Updating meta.json files with correct author image paths...\n');
  ...
  Object.keys(bookAuthors).forEach(bookSlug => {
    ...
    try {
      if (fs.existsSync(metaPath)) {
        ...
        // Update author image paths
        if (metaContent.authors && Array.isArray(metaContent.authors)) {
          metaContent.authors.forEach((author, index) => {
            if (authorsList[index]) {
              const authorImagePath = `/images/books/${bookSlug}/author/${authorsList[index].name.toLowerCase().replace(/\s+/g, '-')}.jpg`;
              author.image = authorImagePath;
              console.log(`  Updated ${author.name} image path: ${authorImagePath}`);
            }
          });
      ...
      console.log(`✗ Error updating ${metaPath}: ${error.message}`);
      errors++;
    }
  });
  
  console.log(`\nResults:`);
  console.log(`✓ Successfully updated: ${updated} meta.json files`);
  console.log(`✗ Errors: ${errors} files`);
  
  return { updated, errors };
}

const result = updateMetaJsonFiles();

if (result.errors === 0) {
  console.log('\n🎉 All meta.json files updated successfully!');
} else {
  console.log('\n⚠️  Some meta.json files could not be updated. Check the errors above.');
}