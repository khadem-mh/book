...
export default function SpecificOfBook({ params }: Props) {
  ...
  return (
    <div className="mx-auto p-6 space-y-8">
          ...
          <Link href={`/books/${slug}/preface`} className="no-underline">
            <div className="group bg-gradient-to-r from-white to-slate-50 rounded-3xl p-6 shadow-lg border-l-4 border-sky-400">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 text-sm text-gray-700">
                <div>
                  <div className="text-xs text-gray-400">نویسندگان</div>
                  <div className="font-medium">{(book.authors ?? []).map((a: any) => a.name).join(", ")}</div>
                </div>

                <div>
                  <div className="text-xs text-gray-400">بلوک‌های مقدمه</div>
                  <div className="font-medium">{book.prefaceBlocksCount ?? "—"}</div>
                </div>
              ...
              <div className="mt-5 text-xs text-gray-500">نکته: مقدمه نمای کلی و خط‌مشی کتاب را نشان می‌دهد — برای درک سریع محتوای کتاب، مقدمه را از دست ندهید.</div>
      ...
      {/* Tags */}
      <div id="tags" className="bg-white rounded-3xl p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-gray-600">
            <div className={`${iconBg} bg-slate-50 text-slate-700`}><TbTag size={18} /></div>
            <div className="text-sm font-medium">تگ‌ها</div>
          </div>

          <div className="text-xs text-gray-400">{(book.tags ?? []).length} تگ</div>
        </div>

        <div className="mt-4 flex flex-wrap gap-3">
          {(book.tags ?? []).map((t: string) => (
            <span key={t} className="text-sm px-4 py-2 bg-slate-100 rounded-full hover:bg-sky-50 transition">{t}</span>
          ))}
        </div>
      </div>
    </div>
